...
resource "azurerm_kubernetes_cluster" "aks" {
  ...
  location            = var.aks_values.location
  resource_group_name = var.aks_values.rg_name
  dns_prefix          = "${random_pet.prefix.id}-k8s"
  kubernetes_version  = var.aks_values.kubernetes_version
  ...
  oms_agent {
    log_analytics_workspace_id      = var.aks_values.log_analytics_workspace_id
